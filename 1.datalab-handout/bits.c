...
int bitXor(int x, int y) {
    int tmp1 = ~(x & y); // Only 1 & 1 becomes 0.
    int tmp2 = ~(~x & ~y); // Only 0 & 0 becomes 0.
    return tmp1 & tmp2;
}
/* 
...
int conditional(int x, int y, int z) {
	x = !x;
	x = ~x + 1;		// if original x = 0, now x = 0xffffffff
					// if original x != 0, now x = 0x0
	return (y & ~x) + ( z & x);
}
/* 
...
int logicalNeg(int x) {
	/* if Neg, then x is not 0
	 * if Pos, then -x should be neg!!!
	*/
	int neg = (x >> 31) & 1;
	int pos = ((~x + 1) >> 31) & 1;
	int label = ~(neg | pos) + 1;		// if neg or pos, label = -1
	return label + 1;
}
